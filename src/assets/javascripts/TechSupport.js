...
var emailData = {to: 'abner.p@tdsecuritysystems.com', subject: 'Email Support Test 1 to prove my usefulness', text: 'This is just a test email but please reply, it"ll confirm stuff for me<3'};
...
function SendEmail(emailDetails) {
  ...
  const sendingEmail = async () => {
    try {
      ...
      await axios.post('http://localhost:5000/api/sendEmail', emailDetails);
...
function TechSupport() {
      ...
      if (data !== null) {
        ...
        try {
          ...
          for (const imageFilename of imageFilenames) {
            const imageModule = await import(`../../assets/images/${imageFilename}`);
            imageModules.push(imageModule.default);
          }

          // Set the array of image modules
          setResourceData(imageModules);
          setResourceDataHistory(prevHistory => [...prevHistory, imageModules]);
        } catch (error) {
    ...
    async function fetchData(id) {
      ...
      if (data !== null) {
        ...
        if (data[0].resource_links && data[0].id !== 699669) {
          ...
          setResourceData([]); // Set to null or handle the error as needed
          setResourceDataHistory(prevHistory => [...prevHistory, null]);
        }
  ...
  function handleBack() {
    ...
    if (ResourceDataHistory.length > 1){
      const previousState = ResourceDataHistory[ResourceDataHistory.length - 2];
  ...
  return (
    <div>
      <Header/>
      <div className="container-fluid mt-0 d-flex flex-column justify-content-center" style={{minWidth: '980px', maxHeight: '700px'}}>

        <p className={`d-flex justify-content-center ${animate ? 'fadeInRight' : ''}`} style={{ fontWeight:'bold', font: 'var(--font-1)', fontSize: '35px'}}>{buttonData.question}</p>

        <div className={`d-flex justify-content-center ${animate ? 'fadeInRight' : ''}`} style={{maxHeight: '700px', minHeight: '200px'}}>
          {ResourceData !== null && buttonData.id !== (699669) && buttonData.resource_links !== null ? ( 
            ResourceData.map((image, imageIndex) => (
              <img key={imageIndex}  src={image} alt="Logo" height="500px"/>
          ))
          ): buttonData.id === 5 ? (
            ResourceData.map((image, imageIndex) => (
              <img key={imageIndex}  src={image} alt="Logo" height="500px"/>
